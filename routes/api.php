<?php

use Illuminate\Http\Request;
use Illuminate\Support\Facades\Route;
use App\Http\Controllers\CompanyManagementController;
use App\Models\CompanyManagement;

/*
|--------------------------------------------------------------------------
| API Routes
|--------------------------------------------------------------------------
|
| Here is where you can register API routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| is assigned the "api" middleware group. Enjoy building your API!
|
*/

Route::middleware('auth:api')->get('/user', function (Request $request) {
    return $request->user();
});

Route::post('company-management',[CompanyManagementController::class, 'store']);
Route::patch('company-management/{id}',[CompanyManagementController::class, 'update']);
Route::get('company-management',[CompanyManagementController::class, 'loadData']);
Route::delete('company-management',[CompanyManagementController::class, 'deleteData']);

// Route::apiResource('company-management', CompanyManagementController::class);